...
for(let i=1; i<=2; i++){
    ...
    //li[i].computedStyleMap.backgrounColor = 'yellow';
}
...
let cssQ = confirm('position:absolute;는 부모의 크기와 위치를 기준으로 위치를 재설정하는 속성이다. 맞으면 확인, 틀리면 취소를 누르세요');
while(cssQ != true){
    cssQ = confirm('position:absolute;는 부모의 크기와 위치를 기준으로 위치를 재설정하는 속성이다. 맞으면 확인, 틀리면 취소를 누르세요');
}alert('정답입니다');
let jsQ = confirm('while은 for문에서 초기값과 증감식을 생략한 것과 다르다. 맞으면 확인, 틀리면 취소를 누르세요');
while(jsQ != false){
    jsQ = confirm('while은 for문에서 초기값과 증감식을 생략한 것과 다르다. 맞으면 확인, 틀리면 취소를 누르세요');
}alert('정답입니다'); */
...
for(let i of li){//querSelectorAll로 생성한 다수 li변수 접근
    console.log(i)
}