...
function scopeTest(){//함수 호출 전 대기상태의 함수 생성구조
    f = 25;//지역변수
    g = 35;//지역변수
    console.log(d+f);//실행 5+25
    console.log(e+g);//실행 15+35
}//블록종료
console.log(f);//실행불가능 25
console.log(g+d);//실행불가능 40
...
function func2(){//함수 대기 상태 func2
    ...
    func1(); //6. func2함수대기 안 func1 함수 호출(func2대기상태로 실행되지 않음)
    console.log(x+y+z);
}
...
//function coffeFunc1(data1){}
//coffeFunc1(10);
//(위)프로그래맹해석 : 함수 생성 후 달라지는 데이터1개에 맞춘 매개변수1개 생성완료. 이후 함수를 호출하고 매개변수에 대입할 10값 입력.
//function coffeFunc1(data1){실행함수+매개변수data1활용}
//coffeFunc1(10);
//매개변수를 생성하고 매개변수에 값을 대입했다면 반드시 함수 내부에 매개변수를 활용한 결과식을 만들어야 한다.
//매개변수 사용 시 호출할 때 매개변수에 전달할 대입값을 작성안해도 함수가 실행되게 하고 싶다면 매개변수 생성 시 기본값 함께 작성해야 합니다. function 함수명(매개변수=기본값){}
...
plusFunc(2);
//호출할때 리턴이 보내 반환값을 묶어주는 실행함수를 별도로 작성한다.
...
function dan99Func(num){
    dan99 = `${num}x1=${num*1} `;
    dan99 += `${num}x2=${num*2} `;//dan99 = dan99(기존 앞에 있는 dan99) + `${num}x2=${num*2}`
